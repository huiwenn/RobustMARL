...
from multiagent.core import World, Agent, Landmark, Satellite, SatWorld
...
# this is the env used for MPEEnv
class MultiAgentPPOEnv(MultiAgentBaseEnv):
    ...
    def _get_agent_noise(self, agent: Agent, shape):
        if agent not in self.noise_dict:
            self.noise_dict[agent] = np.random.normal(0, self.obs_noise_level, shape)
        return self.noise_dict[agent]
    
    def _get_obs(self, agent: Agent) -> np.ndarray:
        obs = self.observation_callback(agent, self.world)
        if self.obs_noise_level > 0:
            obs += self._get_agent_noise(agent, obs.shape)
        return obs
